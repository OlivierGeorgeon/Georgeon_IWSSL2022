...
\figureref{fig:results} reports two different phenomena constructed with two kinds of object: an object that affords echo and shock (top), and an object that also affords line detection (bottom).
...
\begin{figure}[htbp]
	...
	\floatconts
	{fig:results}
	{\caption{Left: the robot and the object seen by the human observer.
			  Right: the phenomenon constructed by the robot is a representation of the object as the robot experiences it through interaction. Top: the object affords echo (orange cells) and schock (red cells). Bottom: the object also affords line detection (black cells). Green cells represent empty space.}}
